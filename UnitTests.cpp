...
TEST (NeuronTest, SpikeWithTwoNeurons) {
	...
	for(size_t i(0); i<925+delay_steps; ++i) {
...
TEST (NetworkTest, ExcitatoryConnections) {
	...
	Network network(2500,10000,2,5);
	unsigned int long nbr_excitatory_connections(0);
...
TEST (NetworkTest, InhibitoryConnections) {
	...
	//Creation of a network with 10000 excitatory neurons and 2500 inhibitory neurons
	Network network(2500,10000,2,5);
	unsigned int long nbr_inhibitory_connections(0);
	...
	*/
	...
	/*Tester le temps réfractaire par exemple ?*/ //+Tester le buffer
