...
//!Constructor
Network::Network(unsigned int long nbr_inhibitory, unsigned int long nbr_excitatory, double eta_, double JI_)
: nbrExcitatory(nbr_excitatory), nbrInhibitory(nbr_inhibitory), clock(0), JI(JI_)
{
	...
	*/
	...
	static std::uniform_int_distribution<>d1(0, nbr_excitatory-1);
	static std::uniform_int_distribution<>d2(nbr_excitatory-1, nbr_tot-1);
...
unsigned int long Network::InhibitoryConnectionsTest(unsigned int long neuronNumber) const {
	...
	for(size_t j(nbrExcitatory-1); j<nbrNeurons; ++j) {			
		/*
		* We look for the presence of the neuron 'neuronNumber' of neurons 
		in targets of the j excitatory neurons of the network
		*/ 
...
void Network::update(double external_current, std::ofstream& output_file) {
	...
	static std::random_device rd;	
	static std::mt19937 gen(rd());
	static std::poisson_distribution<> d(Nu_ext);
		...
		if(spike) {
			...
			for(size_t j(0); j<neurons[i].getTargets().size(); ++j) { 
				...
				if(neurons[i].isExcitatory()) { 
					...
					*/
					neurons[(neurons[i].getTargets())[j]].receiveSpike((getClock())+(delay_steps), JE); 
					...
					*/
					neurons[(neurons[i].getTargets())[j]].receiveSpike((getClock())+(delay_steps), -JI); 
